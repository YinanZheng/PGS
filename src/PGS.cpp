...
// [[Rcpp::export]]
List S_H_E_normal_cpp(vec y_vect, mat x_mat, vec id_vect, mat hat_R, vec beta_val, int n, vec m, double lam){
  int p = x_mat.n_cols;
  int nm = x_mat.n_rows;
  vec m_cum = cumsum(m);
  double sigma = 0;
  ...
  sigma = as<double>(wrap(sqrt(sum(pow(resi, 2))/(nm - p))));
  
  for(int i=0; i<n; i++)
  {
    X = x_mat.rows((m_cum(i) - (m(i)-1)), m_cum(i));
    Y = y_vect.subvec((m_cum(i) - (m(i)-1)), m_cum(i));
    Xh = X.t() * hat_R.i();
    S = S + pow(sigma,-2) * (Xh * (Y - X * beta_val));
    H = H + pow(sigma,-2) * (Xh * X);
  }
  
  E = diagmat(q_scad_cpp(beta_val_abs, lam, 3.7) / (beta_val_abs + 1e-6));
...
// [[Rcpp::export]]
List beta_shrink_normal_cpp(vec y_vect, mat x_mat, vec id_vect, mat hat_R, vec beta_ini, int n, vec m, double lam, int eps_stop){
  int p = x_mat.n_cols;
  int step_up = 0;
  ...
  List S_H_E_val = S_H_E_normal_cpp(y_vect, x_mat, id_vect, hat_R, beta_new, n, m_cum, lam);
  S = as<mat>(S_H_E_val[0]);
  H = as<mat>(S_H_E_val[1]);
  E = as<mat>(S_H_E_val[2]);
  while(flag_stop < eps_stop)
  {
    vec beta_old = beta_new;
    beta_new = beta_old + (H + n * E + (1e-6) * eye(p,p)).i() * (S - n * E * beta_old);
    S_H_E_val = S_H_E_normal_cpp(y_vect, x_mat, id_vect, hat_R, beta_new, n, m, lam);
    S = as<mat>(S_H_E_val[0]);
    H = as<mat>(S_H_E_val[1]);
    E = as<mat>(S_H_E_val[2]);
    step_up++;
...
// [[Rcpp::export]]
List CV_lam_grid_cpp(vec y_vect, mat x_mat, vec id_vect, mat hat_R, vec beta_ini, int n, vec m, int fold, vec lam_vect, int eps_stop){
  ...
  for(int j=0; j<lam_length; j++)
  {
      ...
      for(int k=0; k<fold; k++)
      {
          ...
          beta_train = as<vec>(beta_shrink_normal_cpp(y_train, x_train, id_train, hat_R, beta_ini, n_train, m, lam_temp, eps_stop)[0]);
...
// [[Rcpp::export]]
List est_pgee_grid_cpp(vec y_vect, mat x_mat, vec id_vect, mat hat_R, vec beta_ini, int n, vec m, int fold, vec lam_vect, double eps, int eps_stop, int max_step){
  ...
  List beta_fit = beta_shrink_normal_cpp(y_vect, x_mat, id_vect, hat_R, beta_ini, n, m, lam_sel, eps_stop);
  return List::create(
...
mat corr_est_normal_cpp(vec y_vect, mat x_mat, vec beta_val, int n, std::string corr_str){
  int p = x_mat.n_cols;
  int nm = x_mat.n_rols;
  vec resi = y_vect - x_mat * beta_val;
  // vec temp_resi, ones(m), alphas(m); ones.fill(1);
  double sigma = as<double>(wrap(sqrt(sum(pow(resi, 2))/(nm - p))));
...
// [[Rcpp::export]]
List one_run_grid_cpp(vec y_vect, mat x_mat, vec id_vect, int n, vec m, int pn, int fold, vec lam_vect, int eps_stop, std::string corr_str){
  mat hat_ini(m,m), hat_R_corr; hat_ini.eye();
  vec beta_ini(pn); beta_ini.fill(0);
  vec beta_shrink;
  List beta_fit_indep = est_pgee_grid_cpp(y_vect, x_mat, id_vect, hat_ini, beta_ini, n, m, fold, lam_vect, eps_stop);
  ...
  List beta_fit_corr = est_pgee_grid_cpp(y_vect, x_mat, id_vect, hat_R_corr, beta_ini, n, m, fold, lam_vect, eps_stop);
  return List::create(
